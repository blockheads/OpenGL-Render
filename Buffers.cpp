...
void BufferSet::createBuffers( Canvas &C ) {
    ...
    vSize = numElements * 9 * sizeof(float);
    ...
    if( colors != NULL ) {
        cSize = numElements * 3 * sizeof(float);
        vbufSize += cSize;
    }

    // texture coords
    /*float* tCoords = C.getTexCoords();
    if (tCoords != NULL) {
    ...
    // add in the texcoord data (if there is any)
    /*if (tSize > 0) {
        glBufferSubData(GL_ARRAY_BUFFER, vSize + cSize, tSize , tCoords);
    }*/
