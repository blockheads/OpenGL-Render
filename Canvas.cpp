...
// additionally provides texture data as well
//
// @param p0 first triangle vertex
// @param p1 second triangle vertex
// @param p2 final triangle vertex
// @param txX x coord of our texture coord
// @param txY y coord of our texture coord
///
...
// add texture coordinates for a triangle feace
void Canvas::addTriangleTexCoord(Vertex p0, Vertex p1, Vertex p2) {
    ...
    // tex Coords
    points.push_back(p0.txX);
    points.push_back(p0.txY);

    //normals
    points.push_back(p0.nx);
    points.push_back(p0.ny);
    points.push_back(p0.nz);
    ...
    points.push_back(p1.txX);
    points.push_back(p1.txY);

    points.push_back(p1.nx);
    points.push_back(p1.ny);
    points.push_back(p1.nz);
    ...
    points.push_back(p2.txX);
    points.push_back(p2.txY);

    points.push_back(p2.nx);
    points.push_back(p2.ny);
    points.push_back(p2.nz);

    numElements += 3;  // three vertices per triangle
}


GLfloat Canvas::getParameterVal()
{
    return this->parameterValue;
}

void Canvas::setParameterVal(GLfloat val)
{
...
///
float* Canvas::getTexCoords(void)
{
    int i;
    ...
    if (texCoords.size() > 0) {
        // create and fill a new color array
        texCoordArray = new float[texCoords.size()];
        if (texCoordArray == 0) {
            cerr << "tex coord allocation failure" << endl;
            exit(1);
        }
        for (i = 0; i < texCoords.size(); i++) {
            texCoordArray[i] = texCoords[i];
        }
    }
